...
//Проверка существования индекса на определённый месяц и возврат его значения
...
void TIndexes::saveNew(QString date, qint64 pos){   //date в формате "2020.04.15", pos - значение индекса
    if (checkIndex(date)==-1){          //Проверка, что такого индекса ещё не было
        ...
        in << date + "&" + QString::number(pos) << endl;   //Запись в файл в формате "2020.10&45"
        file.close();                   //Закрытие файла
    }
};
//Обновление индексов. Добавлена запись даты date длиной length. До добавления последняя запись начиналась с position
