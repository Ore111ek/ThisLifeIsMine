...
QString encodeDecode(const char* input, int inputLength, const char* key, int keyLength)
{
    ...
    //input = m_textEdit->toPlainText().toLatin1().data();
    //inputLength = m_textEdit->toPlainText().toLatin1().length();
    //key = m_keyLineEdit->text().toLatin1().data();
    //keyLength = m_keyLineEdit->text().toLatin1().length();
    ...
    return QString::fromUtf8(output, inputLength);
}

//char* XORChiper(char orignalString[], char key[]) {
//   int keylen = strlen(key);
//   int len = strlen(orignalString);
//   int j = 0;
//   for (int i = 0; i < len; i++){
//      orignalString[i] = orignalString[i] ^ key[j%keylen];
//      j++;
//   }
...
void decodeFile(QString filename, QString password){
    QString line;
    QFile file(filename);
    QFile newFile("new" + filename);      //Создаём новый файл, в который перепишем обновлённый журнал
